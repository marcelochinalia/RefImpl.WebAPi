...
namespace WebApiThrottle
{
    ...
    public class ThrottlingHandler : DelegatingHandler
    {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            long firstRateLimit = 0;
            DateTime firstTimeSpan = new DateTime();
            long firstTotalRequests = 0;
            string firstRateLimitPeriod = String.Empty;
            ...
            foreach (var rate in defRates)
            {
                ...
                if (rateLimit > 0)
                {
                    ...
                    // customização Consinco
                    if (rate.Equals(defRates.FirstOrDefault(x => x.Value > 0)))
                    {
                        firstRateLimitPeriod = core.GetPeriodString(rateLimitPeriod);
                        firstRateLimit = rateLimit;
                        firstTimeSpan = throttleCounter.Timestamp + timeSpan;
                        firstTotalRequests = throttleCounter.TotalRequests;
                    }
                    ...
                    if (throttleCounter.TotalRequests > rateLimit)
                    {
                        ...
                        var message = !string.IsNullOrEmpty(this.QuotaExceededMessage) 
                            ...
                            : "Chamadas na API excedidas! Maximo admitido {0} por {1}.";
            ...
            var retorno = base.SendAsync(request, cancellationToken);

            retorno.Result.Headers.Add("X-Rate-Limit-Limit", new string[] { firstRateLimit.ToString() + firstRateLimitPeriod.ToString() });
            retorno.Result.Headers.Add("X-Rate-Limit-Remaining", new string[] { (firstRateLimit - firstTotalRequests).ToString() });
            retorno.Result.Headers.Add("X-Rate-Limit-Reset", new string[] { firstTimeSpan.ToString() });
